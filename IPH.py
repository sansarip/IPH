import argparse
parser = argparse.ArgumentParser(prog="IPSorter",usage="%(prog)s [options]",description="Finds IP addresses, outputs a"
                       " CSV file of your choosing (private, public, or both)\n"+"Example: -p -i file.csv -o file2.csv")# flag types... 'store_true' gives flag a default value (of False) so that it doesn't expect an argument
parser.add_argument("-p",action='store_true',help ="gets private")
...
def IP_Finder():
    ...
    for i in listAll:                                                                                                   # iterates through the list-form of the text
        index += 1
        if i == '.':                                                                                                    # if '.' found, then gets the first three octets of IP
